 ...
 * 2. Junctions @deepseek-ai/dsh-starhub-client-nav into the launcher-maintained
 *    flat fallback $DSH_HOME/profiles/node_modules — the local package is not
 *    in the dsh app's dependency closure, so healProfilesModuleFallback never
 *    links it on its own.
...
const link = join(dshHome, 'profiles', 'node_modules', '@deepseek-ai', 'dsh-starhub-client-nav')
const target = join(repoRoot, 'packages', 'starhub', 'client-nav')
