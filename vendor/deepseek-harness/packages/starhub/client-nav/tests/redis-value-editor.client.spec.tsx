...
function installInvoke(
  ...
  opts?: { getError?: unknown; executeError?: unknown; setError?: unknown },
) {
  const invoke = vi.fn((cmd: string, args?: Record<string, unknown>) => {
    switch (cmd) {
      ...
      case 'db_redis_set': return opts?.setError ? Promise.reject(opts.setError) : Promise.resolve(null)
...
describe('RedisValueEditor empty & load', () => {
  ...
  it('opens a string key, shows loading, and loads its value and TTL', async () => {
    ...
    await waitFor(() => expect((screen.getByPlaceholderText('值…') as HTMLTextAreaElement).value).toBe('hello'))
  ...
  it('stringifies a non-string value for a string-typed key (JSON pretty-printed)', async () => {
    ...
    renderEditor()
    open('foo', 'string')
    await waitFor(() => expect((screen.getByPlaceholderText('值…') as HTMLTextAreaElement).value).toContain('"nested"'))
  })
...
describe('RedisValueEditor string save & revert', () => {
  it('saves an edited string with a TTL and updates dirty state', async () => {
    ...
    expect((screen.getByText('还原') as HTMLButtonElement).disabled).toBe(true)
  })
...
describe('RedisValueEditor structural (hash) rows', () => {
  it('renders hash rows, deletes one, edits another, and saves HDEL + HSET', async () => {
    ...
    // 重命名第二行(b→b2)→ dirty
    const fieldInputs = screen.getAllByPlaceholderText('字段') as HTMLInputElement[]
  ...
  it('reverts a deleted-and-changed hash row back to the original', async () => {
    ...
    expect((screen.getByPlaceholderText('字段') as HTMLInputElement).value).toBe('a')
  })
})
...
describe('RedisValueEditor structural (list/set/zset) rows', () => {
  ...
  it('renders zset rows as [member, score] pairs and saves ZADD with score first', async () => {
    ...
    renderEditor()
    open('z', 'zset')
    await waitFor(() => expect(screen.getAllByPlaceholderText('字段')[0] as HTMLInputElement).toBeTruthy())
  ...
  it('disables the add button when the new-field name is blank', async () => {
    ...
    expect((screen.getByTitle('新增') as HTMLButtonElement).disabled).toBe(true)
  })
})
...
describe('RedisValueEditor structural failure & empty value', () => {
  ...
  it('handles a get_value payload with a null value and null TTL for a structural key', async () => {
    installInvoke({ h: { key: 'h', type: 'hash', value: null, ttl: null as unknown as number } })
    renderEditor()
    open('h', 'hash')
    // rowsFromValue null → 无字段行;ttl null → 回退 -1 → 输入框 placeholder 持久化
...
describe('RedisValueEditor extra branches', () => {
  ...
  it('revert keeps an added (unsubmitted) structural row with its entered field/value', async () => {
    ...
    fireEvent.click(screen.getByText('还原'))
    await waitFor(() => expect((screen.getByPlaceholderText('字段') as HTMLInputElement).value).toBe('newmem'))
  })
  ...
  it('parses a set value whose items include nullish members into blank rows', async () => {
    installInvoke({ s: { key: 's', type: 'set', value: [null, undefined] as unknown as string[], ttl: -1 } })
    renderEditor()
    open('s', 'set')
    // 两条数据行 + 新增行,占位符都是「字段」
    await waitFor(() => expect(screen.getAllByPlaceholderText('字段').length).toBe(2))
    // `pair` 为 null → 空字段与空值
    expect((screen.getAllByPlaceholderText('字段')[0] as HTMLInputElement).value).toBe('')
  })
  ...
  it('maps a hash value with a null field value to a blank cell', async () => {
    ...
    renderEditor()
    open('h', 'hash')
    await waitFor(() => expect((screen.getByPlaceholderText('字段') as HTMLInputElement).value).toBe('a'))
    expect((screen.getAllByPlaceholderText('值')[0] as HTMLInputElement).value).toBe('')
  })
  ...
  it('parses a zset row whose score is nullish into two blank cells', async () => {
    ...
    renderEditor()
    open('z', 'zset')
    await waitFor(() => expect((screen.getByPlaceholderText('字段') as HTMLInputElement).value).toBe('mem'))
    // pair[1] 为 null → 值空串(pair[1] ?? '')
    expect((screen.getAllByPlaceholderText('值')[0] as HTMLInputElement).value).toBe('')
  })

  it('parses a list whose first item is nullish into a blank value cell', async () => {
    installInvoke({ l: { key: 'l', type: 'list', value: [null, 'b'] as unknown as string[], ttl: -1 } })
    renderEditor()
    open('l', 'list')
    await waitFor(() => expect((screen.getAllByPlaceholderText('索引')[0] as HTMLInputElement).value).toBe('0'))
