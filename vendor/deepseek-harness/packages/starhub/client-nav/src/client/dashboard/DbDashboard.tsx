  ...
  return (
    <div className={css.dashboard}>
      ...
      {dbType === 'redis' && (
        <>
          <div className={css.grid} style={tab === 'overview' ? undefined : { display: 'none' }}>
            <DashboardCard title="运行时间" icon={<span>🕒</span>} value={redis.uptimePretty}
            ...
            <DashboardCard title="已用内存" icon={<span>💾</span>} value={redis.usedMemoryHuman}
            ...
            <DashboardCard title="总键数" icon={<span>🔑</span>} value={redis.totalKeys} color="cyan"
              loading={loading} chartValue={redis.totalKeys} sampleKey={sampleKey.current} />
            <DashboardCard title="客户端连接" icon={<span>🔗</span>} value={redis.connectedClients}
          ...
          <div className={css.grid} style={tab === 'performance' ? undefined : { display: 'none' }}>
            <DashboardCard title="命中率" icon={<span>🎯</span>} value={`${redis.hitRate.toFixed(2)}%`}
            ...
            <DashboardCard title="峰值内存" icon={<span>📈</span>} value={formatDbBytes(redis.usedMemoryPeak)}
              color="yellow" loading={loading} chartValue={redis.usedMemoryPeak} sampleKey={sampleKey.current} />
            <DashboardCard title="累计命令数" icon={<span>💬</span>}
            ...
            <DashboardCard title="每秒操作数" icon={<span>⚡</span>} value={redis.instantaneousOpsPerSec}
      ...
      {dbType === 'mysql' && (
        <>
          <div className={css.grid} style={tab === 'overview' ? undefined : { display: 'none' }}>
            <DashboardCard title="运行时间" icon={<span>🕒</span>} value={mysql.uptimePretty}
              subtitle={`${mysql.uptimeSeconds} 秒`} color="cyan" loading={loading}
              chartValue={mysql.uptimeSeconds} sampleKey={sampleKey.current} />
            <DashboardCard title="连接数" icon={<span>🔗</span>} value={mysql.threadsConnected}
            ...
            <DashboardCard title="数据大小" icon={<span>🗄️</span>} value={formatDbBytes(mysql.dataSize)}
            ...
            <DashboardCard title="表数量" icon={<span>📋</span>} value={mysql.tableCount} color="cyan"
          ...
          <div className={css.grid} style={tab === 'performance' ? undefined : { display: 'none' }}>
            <DashboardCard title="累计查询" icon={<span>🔎</span>} value={mysql.queries.toLocaleString()}
            ...
            <DashboardCard title="慢查询" icon={<span>🐢</span>} value={mysql.slowQueries}
            ...
            <DashboardCard title="缓冲池命中率" icon={<span>📦</span>}
            ...
            <DashboardCard title="活跃线程" icon={<span>⚙️</span>} value={mysql.threadsRunning}
          ...
          <div className={css.grid} style={tab === 'network' ? undefined : { display: 'none' }}>
            <DashboardCard title="网络接收" icon={<span>📥</span>} value={formatDbBytes(mysql.bytesReceived)}
              color="blue" loading={loading} chartValue={mysql.bytesReceived} sampleKey={sampleKey.current} />
            <DashboardCard title="网络发送" icon={<span>📤</span>} value={formatDbBytes(mysql.bytesSent)}
      ...
      {dbType === 'postgresql' && (
        <>
          <div className={css.grid} style={tab === 'overview' ? undefined : { display: 'none' }}>
            <DashboardCard title="运行时间" icon={<span>🕒</span>} value={formatDbUptime(postgres.uptimeSeconds)}
            ...
            <DashboardCard title="连接数" icon={<span>🔗</span>} value={postgres.connections}
            ...
            <DashboardCard title="数据库大小" icon={<span>🗄️</span>} value={formatDbBytes(postgres.databaseSize)}
              color="blue" loading={loading} chartValue={postgres.databaseSize} sampleKey={sampleKey.current} />
            <DashboardCard title="当前 Schema 表数" icon={<span>📋</span>} value={postgres.tableCount} color="cyan"
              loading={loading} chartValue={postgres.tableCount} sampleKey={sampleKey.current} />
          </div>
          <div className={css.grid} style={tab === 'performance' ? undefined : { display: 'none' }}>
            <DashboardCard title="活跃会话" icon={<span>⚙️</span>} value={postgres.activeConnections} color="green"
              loading={loading} chartValue={postgres.activeConnections} sampleKey={sampleKey.current}
              detailTable={postgresConnectionTable} />
            <DashboardCard title="慢语句" icon={<span>🐢</span>} value={postgresSlowStatements.length}
            ...
            <DashboardCard title="缓存命中率" icon={<span>📦</span>} value={`${postgres.cacheHitRate.toFixed(2)}%`}
              progress={postgres.cacheHitRate} color="cyan" loading={loading}
              chartValue={postgres.cacheHitRate} sampleKey={sampleKey.current} />
            <DashboardCard title="累计事务" icon={<span>🔁</span>} value={postgres.transactions.toLocaleString()}
