...
`ProducedFiles` renders the row between the closing message's body and its IconActions footer: a quiet label and one measured file lane. It shows the largest leading prefix that fits (up to six chips; basename text, full path as the `title`) while reserving the exact localized `+ N files` width, so the remainder stays visible without wrapping or horizontal scrolling. Each produced entry also carries the change shape the call's diff declared — created-vs-modified (`oldText: null` marks a create/overwrite) and a +/- line estimate from the hunk texts — rendered as a small stat beside the chip's name; clicking the remainder count opens the right-edge drawer (v0.92.1, replacing the v0.91.0 inline expansion): a fixed panel listing every produced file grouped into 新增/修改 sections (one row per file: full path, stats, same opener; sections fold from their headers; Escape, mask click, and × all close it and focus returns to the remainder button). Each chip prefers the owner-supplied `viewFile` — the in-app viewer the tool rows use (`{ kind: 'read', path }`) — and falls back to the owner's `openFile` (Host OS open) when the viewer is absent; the chat view resolves relative paths against the session cwd for both. The drawer's footer carries the **Show in folder** action, which opens the session workspace through that same owner path only while the page is loopback and the current Host handshake reports `canOpenPath`; direct remote Web and headless/container Linux Hosts omit the action by default. Design rationale: the [workspace file links Agent Note](../../../.agents/notes/implemented/feature/2026-07-31-web-workspace-file-links.md).
