...
The overlay reads content through Tauri `local_read_text_file` (256 KB window, truncated flag surfaced as a notice) and writes through `local_write_text_file`. Read mode shows the current file content; Edit mode renders two columns — 变更前 (before) and 变更后 (after) — per hunk. Editing and saving are gated on the session being idle: `useSessions(s => s.byId[target.sessionId]?.running ?? false)` disables the editors and the save button and shows the 「AI 运行中只能查看」 banner while a run is active. Saving a read view writes the edited content directly; saving an edit view splits the after column on the separator, re-reads the latest file, applies each hunk's `oldText → newText` at its first occurrence (`applyDiffs`), and writes the result — hunks with empty `oldText` (pure insertion) or missing anchors are rejected with a message.

The columns render as a colored comparison by default (2026-08-22): `diff-lines.ts` runs a per-hunk line-level LCS (common prefix/suffix trimmed, exact table under a 200k-cell budget, all-changed fallback past it) and only the genuinely changed lines take the red (-) / green (+) block with a sign gutter; lines both sides share stay plain, and hunk boundaries show as dashed dividers instead of the separator text. Editing the after side is a column-head toggle (「编辑」 ↔ 「查看对比」): the comparison view is the default, the toggle swaps the after column to a plain textarea, and switching back re-colors against the current draft. The save path is unchanged — it still applies the separator-joined draft.

The same `viewFile` currency later extended past tool cards to the turn tail (2026-08-22): `TurnTailOwnerProps.viewFile?` carries it to ui-deliverables, where the `ProducedFiles` chips and the closing prose's `chatFileMentions` inline-code links both prefer the in-shell viewer (`{ kind: 'read', path }`) over the OS opener. Because the viewer reads through Tauri with the path verbatim while produced paths are workspace-relative, the chat view's `viewFile` inject now resolves `request.path` against the session cwd (`resolveWorkspacePath`, the same resolution `openFile` already applied) before calling `viewer.open` — a latent fix for relative tool-arg paths as well.
