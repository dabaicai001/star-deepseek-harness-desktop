...
Remote sync joined the panel later (2026-08-22): `git branch -r --format=%(refname:short)` lists remote-tracking refs (symbolic `origin/HEAD` filtered, refs whose local counterpart exists suppressed), and clicking one runs `git checkout -b <name> --track <ref>` — the "pull the online branch down to local" gesture; `git fetch --all --prune` (「同步远程」) and `git pull` (「拉取」) round out the sync verbs, all with the 120-second network timeout. The same change fixed the panel's occlusion: the pill root now creates its own stacking context at `z-index: 9`, above the sticky composer seat (7) and the scroll-to-bottom control (8), so the dropdown's tail no longer slides under the composer's gradient mask, and the panel is viewport-bounded (`max-height: calc(100vh - 96px)`) with the branch list as the only flexible scroll region, keeping the commit/pull/push rows always visible. A follow-up sizing fix widened the panel to `min(340px, calc(100vw - 48px))` and made the three sync buttons equal-share flex items with `white-space: nowrap` — at 280px the labels (拉取(git pull) etc.) wrapped inside their buttons and overflowed.
