    ...
    pub async fn open_shell(
        ...
        *channel_arc.lock().await = Some(channel);
        let mut writer = {
            ...
            guard.as_mut().ok_or("Channel unexpectedly missing after pty+shell")?.make_writer()
        };
        let (write_tx, mut write_rx) = mpsc::unbounded_channel::<Vec<u8>>();
        { let mut ch = channels.lock().await; ch.insert(session_id.to_string(), write_tx); }
        ...
        tokio::spawn(async move {
            loop {
                let msg = { let mut guard = channel_for_read.lock().await; let Some(ch) = guard.as_mut() else { break; }; ch.wait().await };
                match msg {
                    Some(ChannelMsg::Data { data }) => {
                        let _ = app_handle.emit(&format!("ssh:data:{}", id_for_read), String::from_utf8_lossy(&data).to_string());
                    }
            ...
            let mut ch = channels_clone.lock().await; ch.remove(&id_for_read);
    ...
    pub async fn resize(&self, cols: u32, rows: u32) -> Result<(), String> {
        ...
        if let Some(ch) = guard.as_mut() {
            ch.window_change(cols, rows, 0, 0)
                .await.map_err(|e| format!("Failed to send window-change: {}", e))?;
        }
        Ok(())
    }
