 ...
 *
 * 机制:建链后向远端交互 shell 注入一段一次性初始化命令,让 shell 每次
 * 回到 prompt 时通过 OSC 7 转义序列(ESC ] 7 ; <cwd> BEL)上报当前目录;
 * 前端在终端输出流里解析该序列更新 cwd。bash 走 PROMPT_COMMAND,zsh 走
 * precmd_functions;sh/dash/fish 无 hook 机制,注入为无害 no-op,退回
 * pwd 输出逐行解析兜底。
 */
 ...
 * 兼容标准 `file://host/path` 形式与注入命令发出的裸路径形式。
 */
