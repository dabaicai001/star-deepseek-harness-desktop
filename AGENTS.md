...
| 协议 | MIT |
| 立项时间 | 2026-06-04 |
| 当前版本 | v0.62.2(内嵌 AI 助手(SSH/DB 等宿主)# 绑定本机不接通:此前 `#LOCAL` / `#LOCAL-资产` 只在 prompt 里作「参照元数据」,工具仍限于当前宿主,AI 明确回答无法访问本机代码;现在绑定含本机(local 作用域或 local 资产)时,本轮实际接入 `local_*` 工具与本机运行时(文件读取免确认,写操作与 Shell 命令走确认卡),提示词同步说明可用能力与确认规则;宿主自带 local 工具(本地工作区)时不重复追加) |
