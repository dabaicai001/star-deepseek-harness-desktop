...
| 协议 | MIT |
| 立项时间 | 2026-06-04 |
| 当前版本 | v0.61.6(SSH 长命令/AI 执行期间掉线优化:新增固定节拍应用层心跳(15s),无条件周期性刷新 NAT 空闲定时器,弥补 russh keepalive 只在「完全空闲」时发心跳的盲区;另修复后台静默模式偶发回显——交互命令预检正则 `\bsh\b` 误伤 `.sh` 脚本/`bash xx.sh`/`grep sh` 等导致错误回退 PTY) |
