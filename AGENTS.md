...
| 协议 | MIT |
| 立项时间 | 2026-06-04 |
| 当前版本 | v0.92.0(AI 长期记忆自动沉淀接入 `@deepseek-ai/dsh-starhub-memory-sink`,turn-stopping 后 LLM 抽取本轮持久事实经 `starhub/memory.write` 写 ai_memories;「启用长期记忆」「自动沉淀记忆」两开关均默认关闭;「管理记忆」弹窗去除 isTauriRuntime 整体禁用,IPC 失败以错误文本展示。) |
