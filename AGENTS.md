...
| 协议 | MIT |
| 立项时间 | 2026-06-04 |
| 当前版本 | v0.62.1(AI 哨兵命令回显进入 AI 上下文致模型困惑:PTY 路径的 dataBuffer(AI `captureOutput` 与超时兜底输出的来源)原先存原始 chunk,哨兵 printf 的 readline 回显原样混入,AI 看到自己没发过的 `printf '\033]777;...'` 内部命令;回显过滤器改为同时作用于渲染流与 AI buffer(真实 OSC 序列含 ESC 字节不受影响,完成判定照常)) |
